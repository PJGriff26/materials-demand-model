...
Before distribution fitting, the raw intensity data undergoes several preprocessing steps:
1. **Known outlier corrections/removals**: Documented corrections for specific technology-material values (see `src/data_quality.py`)
2. **Technology consolidation**: Some technologies in the raw data are split into cell-specific and balance-of-system (BOS) entries. During preprocessing, these are merged:
   - `CDTE` (15 rows, BOS materials) → merged into `CdTe` (cell-specific materials)
   - `ASIGE` (15 rows, BOS materials) → merged into `a-Si` (cell-specific materials)
   - This ensures CdTe and a-Si have complete material coverage (both cell and structural materials)
   - After consolidation, the pipeline processes 19 technologies (reduced from 21 in raw data)
3. **Statistical outlier filtering**: IQR-based detection within technology-material groups
...
|------------|------------------|
| Solar PV (UPV, distributed, CSP) | 30 |
| Wind (onshore/offshore) | 25 |
| Nuclear (conventional, SMR) | 60 |
| Hydro (conventional, pumped) | 80 |
| Coal (with/without CCS) | 40 |
| Natural Gas (CC, CT, CCS, H2-CT) | 30 |
| Biomass (with/without CCS) | 30 |
| Geothermal | 30 |
| Battery Storage | 15 |
| Electrolyzer | 20 |
